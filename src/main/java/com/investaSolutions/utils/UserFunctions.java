...
public class UserFunctions {
	...
	public void clickLogout() {
		SeleniumUtils.waitForElementClickable(driver, LOGOUT_BUTTON, WAIT_SECONDS).click();
	}

	public String getLogoutText() throws Exception {
		try {
			return SeleniumUtils.waitForElementClickable(driver, LOGOUT_BUTTON, WAIT_SECONDS).getText();
		} catch (Exception e) {
			throw e;
		}
	}
	...
	// fetch the user details based on user
	public void enterDetailsAndLogin(String user) throws Exception {
		try {
			...
			ExtentReportManager.logInfoDetails(
					String.format(properties.getLogMessage("UserLoginSuccessful"), userDetails.get("username")));
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	...
	public boolean enterDetailsAndLoginUsingCredentials(String userName, String password) throws Exception {
		try {
			...
			clickLogin();
			if (SeleniumUtils.waitForElementClickable(driver, LOGOUT_BUTTON, WAIT_SECONDS).isDisplayed()) {
				ExtentReportManager.logInfoDetails(String.format(properties.getLogMessage("UserLoginSuccessful"), userName));
				return true;
			} else {
				return false;
			}
	...
	public void loginUser(String portal, String user) throws Exception {
		try {
			...
			if (portal.equals(TestBase.properties.getConstant("Admin"))) {
				driver.get(ADMIN_ENVIRONMENT);
				enterDetailsAndLogin(user);
			}
		} catch (Exception e) {
			throw e;
